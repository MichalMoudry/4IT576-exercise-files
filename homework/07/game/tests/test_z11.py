...
"""
Modul testuje správné zapracování modifikace s kódem »21w11«,
...
class Visitor_z11(Visitor):
    ...
    def after_game_start(self, scenario:Scenario):
        ...
        self.not_entered = compare_containers.from_game
    ...
    def after_step_test(self, step:ScenarioStep, answer:str):
        ...
        if not (step.typeOfStep in NO_COMMAND):
            ...
            if action_name in self.not_entered:
                self.not_entered.remove(action_name)
        ...
        required = str(self.not_entered)
        if rest != required:
            error('Seznamy nezadaných akcí si neodpovídají',
