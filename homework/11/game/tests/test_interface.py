...
def test(factory:IInterface, ID:Level) -> bool:
    ...
    print(f'Výsledky jednotlivých testů:\n{dbg.prSeq(result, prn=False)}')
...
def _verify_author():
    ...
    try:
        _author_name = _factory.AUTHOR_NAME
        _author_ID   = _factory.AUTHOR_ID
    except Exception:
        ERROR(f'Tovární objekt «{full_name}»\n'
              f'neposkytuje jméno a/nebo identifikační string autora')
    try:
        _author_lang = _factory.AUTHOR_ORIG_NAME
    except Exception:
